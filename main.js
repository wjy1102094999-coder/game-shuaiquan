(() => {
  ...
  function resetGame() {
    ...
    state.wave = 1;
    state.spawnTimer = 3.1;
    state.pickupTimer = 0.8;
    state.gameOver = false;
    state.gameOverTimer = 0;
    state.shake = 0;
    state.hitStop = 0;
    state.nextUpgradeAt = FIRST_UPGRADE_LEVEL;
  ...
  function queueReachedUpgradeMilestones() {
    let offers = 0;
    while (state.level >= state.nextUpgradeAt) {
      offers += 1;
  ...
  function updatePose(fighter, dt) {
    ...
    fighter.parriedTimer = Math.max(0, (fighter.parriedTimer || 0) - dt);
    fighter.parryVulnerableTimer = Math.max(0, (fighter.parryVulnerableTimer || 0) - dt);
  ...
  function tryFistHit(attacker, fist, target, targetBody) {
    ...
    const vulnerableBonus = target.parryVulnerableTimer > 0 && attacker.isPlayer ? 1.4 : 1;
    damageFighter(target, getFistDamage(attacker, fist) * vulnerableBonus, attacker, "fist");
    return true;
  }

  function applyPlayerParryCounter(ally, enemy, contactX, contactY) {
    const allyBody = getBodyCenter(ally);
    const enemyBody = getBodyCenter(enemy);
    let away = normalize(enemyBody.x - allyBody.x, enemyBody.y - allyBody.y);
    if (away.length < 0.001) {
      away = normalize(enemy.x - ally.x, enemy.y - ally.y);
    }

    const parryPower = ally.parryPower || 1;
    const push = 460 + parryPower * 150;
    enemy.vx += away.x * push;
    enemy.vy += away.y * push;
    ally.vx -= away.x * 80;
    ally.vy -= away.y * 80;

    for (const fist of enemy.fists) {
      fist.vx += away.x * (push * 1.15);
      fist.vy += away.y * (push * 1.15);
      fist.hitCooldown = Math.max(fist.hitCooldown || 0, 0.34);
      fist.parryCooldown = Math.max(fist.parryCooldown || 0, 0.36);
    }

    enemy.hitFlash = 0.24;
    enemy.parriedTimer = 0.42;
    enemy.parryVulnerableTimer = 0.9;
    if (enemy.ai) {
      enemy.ai.attackTimer = 0;
      enemy.ai.attackCooldown = Math.max(enemy.ai.attackCooldown || 0, 0.95);
      enemy.ai.lungeTimer = Math.max(enemy.ai.lungeTimer || 0, 0.6);
    }

    state.player.invulnTimer = Math.max(state.player.invulnTimer || 0, 0.52);
    state.hitStop = Math.max(state.hitStop, 0.065);
    state.shake = Math.max(state.shake, 11);
    addDirectionalBurst(contactX, contactY, away.x, away.y, "#f7f1dc", 24, 1.22);
  }
  ...
  function tryFistParry(a, fistA, b, fistB) {
    ...
    const playerParry = a.isPlayer && !b.isPlayer
      ? { ally: a, enemy: b }
      : b.isPlayer && !a.isPlayer
        ? { ally: b, enemy: a }
        : null;
    const radiusScale = playerParry ? 1.16 : 0.92;
    const radius = a.fistRadius * radiusScale + b.fistRadius * radiusScale;
    ...
    const power = (playerParry ? 520 : 260) * ((a.parryPower || 1) + (b.parryPower || 1)) * 0.5;
    fistA.vx += normal.x * power;
    fistA.vy += normal.y * power;
    fistB.vx -= normal.x * power;
    fistB.vy -= normal.y * power;
    a.vx += normal.x * power * (playerParry ? 0.12 : 0.08);
    a.vy += normal.y * power * (playerParry ? 0.12 : 0.08);
    b.vx -= normal.x * power * (playerParry ? 0.12 : 0.08);
    b.vy -= normal.y * power * (playerParry ? 0.12 : 0.08);
    fistA.hitCooldown = Math.max(fistA.hitCooldown || 0, playerParry ? 0.05 : 0.11);
    fistB.hitCooldown = Math.max(fistB.hitCooldown || 0, playerParry ? 0.32 : 0.11);
    fistA.parryCooldown = playerParry ? 0.16 : 0.22;
    fistB.parryCooldown = playerParry ? 0.32 : 0.22;
    if (a.isPlayer || b.isPlayer) {
      state.hitStop = Math.max(state.hitStop, playerParry ? 0.065 : 0.028);
      state.shake = Math.max(state.shake, playerParry ? 11 : 5);
      if (state.upgrades.healOnParry > 0) {
        healFighter(state.player, state.upgrades.healOnParry);
      }
    }
    const contactX = (fistA.x + fistB.x) * 0.5;
    const contactY = (fistA.y + fistB.y) * 0.5;
    if (playerParry) {
      applyPlayerParryCounter(playerParry.ally, playerParry.enemy, contactX, contactY);
    }
    addBurst(contactX, contactY, "#f7f1dc", playerParry ? 24 : 12, playerParry ? 1.05 : 0.62);
    return true;
  }
  ...
  function updateSpawn(dt) {
    ...
    if (nextWave !== state.wave) {
      state.wave = nextWave;
      state.spawnTimer = state.wave <= 3 ? Math.max(state.spawnTimer, 1.45) : Math.min(state.spawnTimer, 0.08);
    ...
    const earlyBase = state.wave <= 2 ? 3 : state.wave <= 4 ? 4 : 7;
    const wavePressure = state.wave <= 4 ? Math.floor(state.wave * 0.5) : Math.floor(state.wave * 1.28);
    const scorePressure = Math.floor(state.score / (state.wave <= 4 ? 12 : 5));
    const targetCap = state.wave <= 2 ? 5 : state.wave <= 4 ? 8 : 24;
    ...
    if (state.spawnTimer <= 0 && state.enemies.length < targetCount) {
      ...
      state.spawnTimer =
        state.wave <= 3
          ? Math.max(2.25, 3.45 - state.wave * 0.2)
